...
for root, dirs, files in os.walk('final/tr_groups'):
    for chunk_idx in files:
        ...
        model_path = 'final/1L_ada/'+str(chunk_idx) 
        if os.path.isfile(model_path):
            ...
            chunk_model = AdaBoostChunkClassifierWithKFolds(k=3,seeds=[13,11,193])
...
check STD and MEAN
'''
for root, dirs, files in os.walk('final/1L_ada'):
    for model_idx in files:
        model = read_variable('final/1L_ada/'+str(model_idx) )
