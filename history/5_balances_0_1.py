...
while True:
    ...
    #####################balance response 0 and 1########################
    tr_y_b = np.ones(tr_nu*2,dtype=np.int)
    tr_x_b = np.empty((tr_nu*2, col_num_date+col_cate))
    
    tr_y_b[:tr_nu] = tr_y
    tr_x_b[:tr_nu,:] = tr_x

    ones = []
    for i, value in enumerate(tr_y):
        if value==1:
            ones.append(tr_x[i,:])
    
    ones_len = len(ones)
    i_start = tr_nu
    i_end = tr_nu+ones_len
    while True:
        tr_x_b[i_start:i_end,:] = ones[0:i_end-i_start]
        i_start = i_end
        i_end = i_start+ones_len
        if i_end >= 2*tr_nu:
            i_end = 2*tr_nu
        if i_start == i_end:
            break
    ...
    del tr_nu,tr_y_b, tr_x_b ,best_model_mcc, best_model, x, y, votes_date_nu
