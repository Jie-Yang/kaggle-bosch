...
#%%
class RandomForestClassifierWithSeeds:
    def __init__(self, seeds):
        self.seeds = seeds
    def fit(self, X, Y, votes_threshold=None):
        self.models = []
    ...
    def __str__(self):
        return "RandomForestClassifierWithSeeds(seeds="+str(self.seeds)+',votes_threshold='+str(self.votes_threshold)+')'


#%%
class ForestChunkClassifierWithKFolds:
    def __init__(self, k, seeds, votes_threshold=None):
        self.k = k
        self.seeds = seeds
        self.votes_threshold = votes_threshold
        if self.votes_threshold is None:
    ...
    def fit(self, X, Y, test_X, test_Y):
        ...
        for tr_idx, val_idx in skf.split(X, Y):
            ...
            model =  RandomForestClassifierWithSeeds(seeds=self.seeds)	
            model.fit(tr_X,tr_Y)
    ...
    def predict(self,X):
        ...
        return (Y_sum>self.votes_threshold).astype(np.int)
    def __str__(self):
        return "ChunkForestClassifierWithKFolds(kfold="+str(self.k)+",seeds="+str(self.seeds)+',votes_threshold='+str(self.votes_threshold)+')'
